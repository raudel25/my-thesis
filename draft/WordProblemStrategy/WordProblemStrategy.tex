...
existe una constante $n$ tal que para toda \comment{\comment{cadena $q\in L$}{que tengan una longitud mayor que una constante... ¿no?.}}{en \cite{authomataTheory} dicen exactamente esto}, $w$ puede escribirse de la forma $q=uvwxy$ tal que:
...
\comment{\notaparaelautor{¿y cómo usaste el hecho de que la cadena es satisfacible? porque a lo mejor las cadenas que corresponden a fórmulas satisfacibles tienen alguna caracerística especial que sí hagan que sean libres del contexto...}}{No hace falta para que la formula sea satisfacible tiene q cumplir que todas las cláusulas tengan la misma longitud y q la formula sea satisfacible. Supongamos que es libre del contexto entonces no puede cumplir la primera condicion}
...
por \comment{\comment{$w$}{¿w con d o sin d? ¿qué significa que una cláusula sea satisfacible por una cadena w que tenga d?}}{$w$ es una cadena binaria sin $d$}. 
...
\comment{por $w$}{¿w con d o sin d? ¿qué significa que una cláusula sea satisfacible por una cadena formada por 0 y 1?}, Lema \ref{lem:clause}. \notaparaelautor{No me convence que se demostró el caso base :-(. Hay muchas cosas en el aire :-(.}
...
El conjunto de todas las fórmulas booleanas satisfacibles por \comment{\comment{ $w$ con $k+1$}{la longitud de w (y la cantidad de d) determinan la cantidad de cláusulas que puede tener una fórmula para que w la satisfaga, a no ser que w no tenga d entonces no sé de qué estamos hablando.}}{$w$ es un cadena binaria sin $d$} cláusulas es equivalente al conjunto que forman todas las fórmulas booleanas satisfacibles por $w$ con $k$ cláusulas concatenadas con todas las cláusulas satisfacibles por $w$:
