...
\agregaesto{AGREGAR INTRODUCCIÓN DE LA SECCIÓN. NO TIENE QUE SER AHORA, PERO DEBEMOS HACERLO, ESTO ES UNA INSTANCIA DE EMPEZAR UNA COSA CON UNA SUBCOSA.}
...
\agregaesto{AGREGAR INTRODUCCIÓN ANTES DE LA SUBCOSA}
...
\notaparaelautor{Me imagino que sí, pero ¿vamos a usar todas esas operaciones? La idea es solo poner aquí las que vayamos a usar.}
...
\paragraph{Problema de la palabra:} Consiste en determinar si una cadena pertenece a un lenguaje dado. Todo problema en Ciencias de la Computación puede ser reducido a un problema de la palabra \agregaesto{[REF]}, ya que cualquier problema
puede ser codificado como un lenguaje formal \agregaesto{[REF]}.
\paragraph{Problema del vacío:} Consiste en determinar si un lenguaje es vacío.

\notaparaelautor{Un ejemplito vendría bien.}
...
Una \textbf{gramática} es un \cambio{sistema matematico}{formalismo} utilizado para describir lenguajes formales. Se define como una 4-tupla:
\[
      G = (N, \Sigma, P, S),
\]
donde:
...
\notaparaelautor{Antes de poner el lenguaje generado por la gramática debemos poner la definición de derivación. la buena noticia es que en este documento podemos poner las definiciones «duras» sin dar explicaciones, ni ejemplitos ni ná :-P.}
...
\begin{enumerate}
      \item \textbf{Tipo 0: Gramáticas irrestrictas}
            \begin{itemize}
                  ...
                  \item Generan los \comment{\textbf{lenguajes recursivamente enumerables}}{Esto harbía que definirlo, aunque sea diciendo que es todo lo que se genera a partir de una gramática irrestricta.}.
            \end{itemize}
            
      \item \textbf{Tipo 1: Gramáticas \comment{\cambio{sensibles al}{dependientes del}}{Esta es la palabra usada en español :-)} contexto}
            \begin{itemize}
                  ...
                  \item Generan los \textbf{lenguajes \cambio{sensibles al}{dependientes del} contexto}. \notaparaelautor{Igual estos lenguajes habría que definirlos, o mencionarlos antes... Es solo para que lector no lance \texttt{null-reference-exceptions} mientras lea el documento.}
            \end{itemize}
      ...
      \item \textbf{Tipo 2: Gramáticas libres de contexto}
            \begin{itemize}
                  ...
                  \item Generan los \comment{\textbf{lenguajes libres de contexto}}{Esto habría que definirlo antes, oooooooooo, cambia el texto y decir, los lenguajes que se pueden generar a partir de una gramática libre del contexto se llaman lenguajes libres del contexto. Ese cambio lo arregla todo por ahí para allá arriba.}.
            \end{itemize}
      ...
      \item \textbf{Tipo 3: Gramáticas regulares}
            \begin{itemize}
                  ...
                  \item Generan los \comment{\textbf{lenguajes regulares}}{Definir antes o cambiar el texto para que no haya que definirlo.}.
            \end{itemize}
\end{enumerate}
...
Una máquina de Turing \comment{consiste la definición formal}{Creo que aquí falta algo} de algoritmo en Ciencias de la Computación y es el eje central para la resolución de problemas. \agregaesto{¿por qué?}
...
\begin{itemize}
      ...
      \item \textbf{Notación $\Theta(f(n))$}: Una función $g(n)$ pertenece a $\Theta(f(n))$ si:
            ...
            \]
            Es decir, $f(n)$ acota \agregaesto{¿a?} $g(n)$ tanto superior como inferiormente.
\end{itemize}
...
Un problema es no decidible si no existe una Máquina de Turing que pueda resolverlo correctamente para todas las entradas posibles. Esto significa que no hay algoritmo que garantice una respuesta en tiempo finito en todos los casos. Un ejemplo clásico de problema no decidible es el \textit{Problema de la Parada} \agregaesto{[REF]}, que consiste en determinar si una Máquina de Turing se detendrá para una entrada dada.
...
computación \agregaesto{[REF]}. Hasta la fecha, se desconoce si $\text{P} = \text{NP}$ o si $\text{P} \neq \text{NP}$. Por otro
lado el conjunto de problemas NP-Completo brinda una base sólida para el problema anterior, ya que dada su
definición\agregaesto{COMA} cualquier problema perteneciente a este conjunto que sea soluble en tiempo polinomial
implica que todos los problemas en NP lo son. \comment{Además}{En vez de un «además» yo pondría algo como por otra parte... porque el además, da idea de que está directamente relacionado con lo anterior... y lo que sigue es medio que un cambio de tema... ¿no?}, existen problemas que no están en P ni en NP, \comment{como
  los problemas indecidibles}{Y los exponenciales y otra pila más. Creo que ahí lo importante es resaltar que para algunos problemas ni siquiera hay algoritmos.}.
...
\begin{itemize}
      ...
      \item \(\delta: Q \times \Sigma \to Q \times \Gamma^*\) es la función de transición, que \comment{mapea}{esto no existe en español, y aún si existiera es preferible usar otra más \textit{formal}} una combinación de estado actual y símbolo de entrada a un nuevo estado y una salida.
...
Un autómata regular se puede representar como una 5-tupla $$\mathcal{A} = (Q, \Sigma, \delta, q_0, F)\agregaesto{COMA}$$ donde:
...
\notaparaelautor{Quizás sea mejor presentar el autómata finito antes del transductor... ¿no te parece?}
...
Un autómata de pila procesa una cadena de entrada desde el estado inicial $q_0$ y puede utilizar transiciones $\varepsilon$ (sin consumir entrada). En cada paso, la función $\delta$ determina el nuevo estado, los símbolos que se \cambio{empujan}{insertan} o \cambio{desapilan}{sacan de la pila}, y el avance en la entrada. Si\quitaesto{,} tras procesar toda la cadena, el autómata termina en un estado de aceptación o con una pila vacía (dependiendo del criterio de aceptación), la cadena es aceptada.

\agregaesto{FALTA TRANSICIÓN A LO QUE SIGUE}

\notaparaelautor{Uffff... y la transición a lo que sigue aquí sí está fuerte...}
...
\notaparaelautor{Ufff. creo que aquí sí deberíamos poner ejemplito...}    
...
\notaparaelautor{Aquí también deberíamos tener un ejemplo. Es para ayudar al (pobre e infeliz) lector.}
...
de memoria al proceso de derivación, esta característica permite la generación de lenguajes más generales que los generados por las CFG \comment{\agregaesto{[REF]}}{Yo pondría la referencia aquí y no allá arriba. o aquí pondría otra.}.
...
en cada producción se puede realizar una operación de \comment{push o pop  en}{insertar o elminiar de} la pila o dejarla en su estado actual. \comment{Esta pila se maneja de forma global
lo cual las diferencia de las gramáticas indexadas \cite{globalIndexLanguages} brindándoles una mayor expresividad. }{yo quitaría esto porque si no vamos a usar nada de las gramáticas indexadas es más ruido que lo que aporta realmente...}
...
en cada paso de la derivación se puede realizar una operación de \comment{push o pop}{cámbialo al español} en la pila, además de las operaciones de sustitución de símbolos.
...
\notaparaelautor{Si ponemos esto aquí dberíamos tenerlo también con las matriciales.}
...
Las gramáticas de concatenación de rango (\textit{RCG}) \cite{mainRCGBib} son un formalismo de gramáticas desarrollado para describir lenguajes más generales que los \quitaesto{generados por gramaticas} libres del contexto.
Este formalismo extiende las capacidades descriptivas \comment{al incluir relaciones entre rangos de la cadena de una manera más flexible}{¿qué esto? :-o},
permitiendo la generación de lenguajes \cambio{sensibles al}{dependientes del} contexto.
...
A continuación se describen las principales propiedades de las RCG \agregaesto{[REF]}:
\begin{itemize}
...
es polinomial \agregaesto{[REF]}, por ejemplo puede pasar que se instancien argumentos de en los predicados con rangos que no pertenezcan
