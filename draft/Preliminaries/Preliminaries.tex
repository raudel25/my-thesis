...
Un problema se clasifica como $\mathsf{NP-completo}$ si satisface dos condiciones esenciales:

\begin{enumerate}
      \item \textbf{Pertenencia a $\mathsf{NP}$:} El problema está en la clase $\mathsf{NP}$, lo que significa que, dada una solución candidata, es posible verificar su corrección en tiempo polinómico con respecto al tamaño de la entrada.

      \item \textbf{NP-dureza:} El problema es tan difícil como cualquier otro problema en $\mathsf{NP}$. Esto significa que cualquier problema en $\mathsf{NP}$ puede reducirse a este problema en tiempo polinómico.
\end{enumerate}

\subsection{SAT como Problema NP}

El SAT es el primer problema demostrado como $\mathsf{NP-completo}$ y juega un rol central en la teoría de la complejidad computacional. Se define en la clase NP porque, dada una asignación de valores a las variables de la fórmula booleana, se puede verificar en tiempo polinómico si dicha asignación satisface la fórmula.

Además, la prueba de que SAT es $\mathsf{NP-completo}$ fue una de las contribuciones principales de Stephen Cook en 1971, marcando el inicio de la teoría de la NP-completitud.
